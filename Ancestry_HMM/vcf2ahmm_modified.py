    ...
    for line in csv.reader(tsv, delimiter="\t") :
        ...
        for s in range(9, len(line)):
            if sample_files[s] in sample2pop:
                pop = sample2pop[sample_files[s]]
                if pop not in totals:
                    totals[pop] = 0
                    alts[pop] = 0

                genotype = line[s].split(':')[0]  # Extract the genotype part
                alleles = genotype.replace('|', '/').split('/')  # Replace '|' with '/' for uniformity

            # Count alleles
                for allele in alleles:
                    if allele == '1':
                        alts[pop] += 1
                    if allele in ['0', '1']:  # Count only if allele is 0 or 1
                        totals[pop] += 1
...
#            continue
        ...
        for s in range(9, len( line ) ) :
            ...
            if ( sample_files[s] in sample2pop and sample2pop[sample_files[s]] == "admixed" ) :
                # Print genotypes
                    if args.g == 1:
                        genotype = line[s].split(':')[0].replace('|', '/').split('/')
                        ref_count = genotype.count('0')
                        alt_count = genotype.count('1')


                        if len(genotype) == 2:  # Diploid genotypes
                            print("\t", 2 - ref_count - alt_count, "\t", alt_count, end="", sep="")
                        else:  # Handle cases where genotype information might be missing or unclear
                            print("\t", 0, "\t", 0, end="", sep="")
